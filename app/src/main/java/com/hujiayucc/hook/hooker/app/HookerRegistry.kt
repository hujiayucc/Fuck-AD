...
private typealias HookerFactory = () -> Hooker

object HookerRegistry {
    private val appHookers = mapOf<String, List<HookerFactory>>(
    ...
    fun create(packageName: String): List<Hooker> {
        return appHookers[packageName]?.map { createHooker -> createHooker() }.orEmpty()
    }
}
